...
function decisionReport(data) {
  ...
  return card('1 — Validação', alternatives.map(a => `<p><b>${escapeHtml(a.nome)}: ${a.viavel === false ? 'INVIÁVEL' : 'VIÁVEL'}.</b> ${a.viavel === false ? escapeHtml(a.motivoInviabilidade || 'Motivo não informado.') : 'Nenhuma violação foi declarada.'}</p>`).join(''), true)
    + card('2–6 — Método, normalização, risco e TCO', `<p>Normalização min–max em 0–100, invertida para “menor é melhor”; empates recebem 100 para evitar divisão por zero. Aplicou-se análise multicritério ponderada. Não há dados de capacidades ou fluxos para programação linear, inteira/mista, knapsack, alocação ou transporte; AHP/TOPSIS exigiriam dados ou opção metodológica adicionais.</p><p>Risco financeiro = probabilidade × impacto. TCO = preço + custos adicionais + risco financeiro − valor residual. Componentes ausentes não foram estimados.</p>`, true)
    + ranked
    + card('8 — Sensibilidade', `<div class="scenario-grid"><p class="scenario"><b>A — Econômico</b><br>${escapeHtml(winners[0] || 'Sem opção viável')}</p><p class="scenario"><b>B — Equilibrado</b><br>${escapeHtml(winners[1] || 'Sem opção viável')}</p><p class="scenario"><b>C — Performance</b><br>${escapeHtml(winners[2] || 'Sem opção viável')}</p></div>${sensitive ? '<p class="alert"><b>RESULTADO SENSÍVEL AOS PESOS.</b></p>' : ''}`, true)
    + card('9–10 — Recomendação e confiança', `<p><b>OPÇÃO RECOMENDADA / MELHOR CUSTO-BENEFÍCIO:</b> ${escapeHtml(winners[1] || 'Indisponível')}<br><b>MENOR PREÇO:</b> ${escapeHtml(viableByPrice[0]?.nome || 'Indisponível')}<br><b>MENOR RISCO:</b> ${escapeHtml(viableByRisk[0]?.nome || 'Indisponível')}<br><b>MELHOR PERFORMANCE:</b> ${escapeHtml(winners[2] || 'Indisponível')}</p>${tie ? '<p class="alert"><b>EMPATE TÉCNICO.</b></p>' : ''}<p><span class="confidence">80%</span> Confiança baseada na completude numérica, sem validação externa. Evidências técnicas, composição integral do TCO, SLA e histórico dos fornecedores aumentariam a precisão.</p>${invalid.some(a => !a.motivoInviabilidade) ? '<p class="hypothesis"><b>DADO AUSENTE:</b> justifique toda inviabilidade.</p>' : ''}`, true);
}
...
function buildModel(data) {
  ...
  const feasibility = mandatory.length ? `Eliminar qualquer alternativa que exceda limites informados ou deixe de atender a um requisito obrigatório. ${mandatory.join(' ')}` : 'Eliminar qualquer alternativa que descumpra requisitos obrigatórios.';
  ...
  return [
    ...
    card('Função objetivo', '<p>Minimizar o custo total entre as alternativas viáveis e maximizar a aderência ponderada aos critérios de avaliação.</p><p class="hypothesis"><b>HIPÓTESE:</b> A decisão será baseada nas alternativas informadas e nos critérios declarados.</p>'),
    card('Restrições', list(restrictions), true), card('Critérios de avaliação', table, true),
    card('Método de otimização', '<p><b>Combinação de métodos:</b> programação inteira mista para selecionar quantidades sob orçamento e limites; weighted scoring para comparar os critérios.</p>', true),
    card('Dados necessários para cálculo', list(['Preço unitário, impostos, frete e demais componentes do custo total.', 'Prazo e capacidade de fornecimento.', 'Especificações técnicas e evidências de desempenho.'])),
    card('Regra de viabilidade', `<p>${feasibility}</p>`),
    card('Nível de confiança', `<p><span class="confidence">BAIXA</span> Há uma descrição inicial, mas ainda não existem alternativas, cotações nem evidências de fornecedores para executar uma análise completa.</p>`)
  ].join('');
}
